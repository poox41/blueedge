...
function PairEditor({ title, value, onChange }: { title: string; value: string; onChange: (value: string) => void }) {
  ...
  return <div className="grid grid-cols-[120px_minmax(0,1fr)] items-start gap-3"><Label className="pt-2 text-right">{title}</Label><div className="min-w-0 space-y-2">{rows.map((row, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_36px] gap-2"><Input value={row.key} placeholder="键" onChange={(event) => commit(rows.map((item, i) => i === index ? { ...item, key: event.target.value } : item))} /><Input value={row.value} placeholder="值" onChange={(event) => commit(rows.map((item, i) => i === index ? { ...item, value: event.target.value } : item))} /><Button variant="ghost" size="icon" onClick={() => commit(rows.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></Button></div>)}<Button type="button" variant="ghost" size="sm" className="px-1 text-[#165DFF] hover:bg-transparent" onClick={() => commit([...rows, { key: `key-${rows.length + 1}`, value: "value" }])}><Plus className="mr-1 h-4 w-4" />添加</Button></div></div>;
}

function FieldRow({ label, required = false, help, children }: { label: string; required?: boolean; help?: string; children: React.ReactNode }) {
  return <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3"><Label className="pt-2 text-right">{label}{required && <span className="ml-1 text-[#F53F3F]">*</span>}</Label><div className="min-w-0">{children}{help && <p className="mt-1.5 text-xs text-[#86909C]">{help}</p>}</div></div>;
}
...
export function DeploymentCreateWizard(props: Props) {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-[#F7F8FA]">
    ...
    <div className="shrink-0 border-b bg-white px-6 py-5"><div className="mx-auto flex max-w-xl items-start">{steps.map((label, index) => <div key={label} className="flex flex-1 items-start last:flex-none"><div className="flex flex-col items-center gap-2"><div className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${index < step ? "border border-[#3491FA] text-[#3491FA]" : index === step ? "bg-[#165DFF] text-white" : "bg-[#E5E6EB] text-[#86909C]"}`}>{index < step ? <Check className="h-3.5 w-3.5" /> : index + 1}</div><span className={`whitespace-nowrap text-xs ${index === step ? "text-[#165DFF]" : "text-[#4E5969]"}`}>{label}</span></div>{index < 2 && <div className={`mx-3 mt-3 h-px flex-1 ${index < step ? "bg-[#3491FA]" : "bg-[#C9CDD4]"}`} />}</div>)}</div></div>
    <main className="min-h-0 flex-1 overflow-y-auto p-6">
      ...
      {false && step === 1 && <section className="mx-auto max-w-6xl space-y-3">
        ...
        <details className="rounded-lg border bg-white p-4"><summary className="cursor-pointer font-medium">健康检查 <OptionalBadge /></summary><div className="mt-5 space-y-5">{([ ["livenessEnabled", "livenessPath", "livenessPort", "容器存活检查"], ["readinessEnabled", "readinessPath", "readinessPort", "容器就绪检查"], ["startupEnabled", "startupPath", "startupPort", "容器启动检查"] ] as const).map(([enabledKey, pathKey, portKey, label]) => <div key={enabledKey} className="grid max-w-4xl grid-cols-[180px_1fr_180px] gap-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form[enabledKey]} onChange={(event) => patch(enabledKey, event.target.checked)} className="h-4 w-4 accent-[#165DFF]" />{label}</label>{form[enabledKey] ? <><Input value={form[pathKey]} onChange={(event) => patch(pathKey, event.target.value)} placeholder="HTTP 路径" /><Input type="number" min={1} max={65535} value={form[portKey]} onChange={(event) => patch(portKey, Number(event.target.value))} placeholder="端口" /></> : <div className="col-span-2" />}</div>)}</div></details>
