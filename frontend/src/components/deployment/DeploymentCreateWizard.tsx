...
export function DeploymentCreateWizard(props: Props) {
  ...
  const validation = useRequiredFieldValidation<"yaml" | "name" | "namespace" | "replicas" | "containerName" | "containerImage">();
  ...
  const goNext = () => {
    ...
    if (step === 0 && !validation.validate([
      { field: "name", valid: validWorkloadName, message: form.name.trim() ? "名称仅支持小写字母、数字和中横线，且须以字母或数字开头结尾" : "请输入负载名称", elementId: "deployment-wizard-name" },
      { field: "namespace", valid: Boolean(form.namespace.trim()), message: "请选择命名空间", elementId: "deployment-wizard-namespace" },
      { field: "replicas", valid: Number(form.replicas) > 0, message: "实例数必须大于 0", elementId: "deployment-wizard-replicas" },
    ])) return;
    if (step === 1) {
      const index = form.containers.findIndex((item) => !item.name.trim() || !item.image.trim());
      const invalid = index >= 0 ? form.containers[index] : undefined;
      if (!validation.validate([
        { field: "containerName", valid: Boolean(invalid?.name.trim() ?? form.containers.length), message: "请输入容器名称", elementId: `deployment-wizard-container-name-${Math.max(index, 0)}` },
        { field: "containerImage", valid: Boolean(invalid?.image.trim() ?? form.containers.length), message: "请输入容器镜像", elementId: `deployment-wizard-container-image-${Math.max(index, 0)}` },
      ])) return;
    }
    setStep((current) => current + 1);
  };
  const submitYaml = () => {
    if (!validation.validate([{ field: "yaml", valid: Boolean(props.yamlText.trim()), message: "请输入 YAML 内容", elementId: "deployment-wizard-yaml" }])) return;
    props.onSubmit();
  };

  if (props.mode === "yaml") {
    return <div className="flex h-full min-h-0 flex-col bg-[var(--color-bg-soft)]"><header className="flex h-14 items-center justify-between border-b bg-white px-6"><div className="flex items-center gap-3"><Button variant="ghost" size="icon" onClick={props.onCancel}><ArrowLeft className="h-5 w-5" /></Button><h2 className="text-lg font-semibold">YAML 创建部署</h2></div><Button variant="outline" onClick={() => props.onModeChange("form")}>切换到表单创建</Button></header><main className="min-h-0 flex-1 p-6"><Textarea id="deployment-wizard-yaml" value={props.yamlText} onChange={(event) => { props.onYamlChange(event.target.value); validation.clearError("yaml"); }} aria-invalid={Boolean(validation.errors.yaml)} className="h-full min-h-[600px] resize-none bg-white font-mono text-xs" /><RequiredFieldError id="deployment-wizard-yaml-error" message={validation.errors.yaml} /></main><footer className="flex h-16 items-center justify-end gap-3 border-t bg-white px-6"><Button variant="ghost" onClick={props.onCancel}>取消</Button><Button onClick={submitYaml} disabled={props.submitting}>创建</Button></footer></div>;
  }
  ...
  return <div className="flex h-full min-h-0 flex-col bg-[var(--color-bg-soft)]">
    ...
    <main className="min-h-0 flex-1 overflow-y-auto p-6">
      ...
      {step === 0 && <section className="mx-auto min-h-[520px] max-w-6xl bg-white p-8 shadow-sm"><div className="grid max-w-3xl grid-cols-[140px_1fr] gap-x-5 gap-y-5"><Label className="pt-2 text-right">负载名称 *</Label><div><Input id="deployment-wizard-name" value={form.name} onChange={(event) => { patch("name", event.target.value); validation.clearError("name"); }} aria-invalid={Boolean(validation.errors.name)} placeholder="最长 63 个字符，小写字母、数字或中划线" /><p className="mt-1 text-xs text-[var(--color-text-tertiary)]">必须由小写字母、数字或中划线组成，并以字母或数字开头及结尾。</p><RequiredFieldError id="deployment-wizard-name-error" message={validation.errors.name} /></div><Label className="pt-2 text-right">负载别名<OptionalBadge /></Label><Input value={form.alias} onChange={(event) => patch("alias", event.target.value)} /><Label className="pt-2 text-right">命名空间 *</Label><div><Select value={form.namespace} onValueChange={(value) => { patch("namespace", value); validation.clearError("namespace"); }}><SelectTrigger id="deployment-wizard-namespace" aria-invalid={Boolean(validation.errors.namespace)}><SelectValue /></SelectTrigger><SelectContent>{props.namespaces.filter((item) => item.value !== "all").map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select><RequiredFieldError id="deployment-wizard-namespace-error" message={validation.errors.namespace} /></div><Label className="pt-2 text-right">实例数 *</Label><div><Input id="deployment-wizard-replicas" type="number" min={1} value={form.replicas} onChange={(event) => { patch("replicas", Number(event.target.value)); validation.clearError("replicas"); }} aria-invalid={Boolean(validation.errors.replicas)} /><RequiredFieldError id="deployment-wizard-replicas-error" message={validation.errors.replicas} /></div><Label className="pt-2 text-right">描述<OptionalBadge /></Label><Textarea className="min-h-28" value={form.description} onChange={(event) => patch("description", event.target.value)} /></div></section>}
