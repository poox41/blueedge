...
export function VolumeEditor({ values, onChange }: { values: VolumeForm[]; onChange: (values: VolumeForm[]) => void }) {
  ...
  return <div className="rounded-lg border border-[var(--color-border)] bg-white p-4 space-y-3">
    <div className="flex items-center justify-between"><div><div className="font-medium text-sm">数据卷 Volumes <span className="ml-2 rounded border border-[var(--color-border)] bg-[var(--color-bg-soft)] px-1.5 py-0.5 text-[10px] font-normal text-[var(--color-text-tertiary)]">选填</span></div><p className="mt-1 text-xs text-[var(--color-text-tertiary)]">先定义卷，再在工作容器或初始化容器中按名称挂载。</p></div><Button type="button" variant="outline" size="sm" onClick={add}><Plus className="mr-1 h-3.5 w-3.5"/>添加数据卷</Button></div>
    {values.map((item) => <div key={item.id} className="grid grid-cols-2 gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-soft)] p-3">
      ...
      <button type="button" className="action-button is-danger col-span-2 justify-self-end" title="删除数据卷" aria-label="删除数据卷" onClick={() => onChange(values.filter((value) => value.id !== item.id))}><Trash2 className="h-3.5 w-3.5"/></button>
    </div>)}
  </div>;
}
