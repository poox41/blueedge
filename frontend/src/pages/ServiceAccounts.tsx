...
import { serviceAccountsData, namespaces } from "@/data/mockData";
import { cn } from "@/lib/utils";

interface SA { namespace: string; name: string; secrets: string; createdAt: string; automount?: boolean; }

const fullData: SA[] = (serviceAccountsData as any[]).map(s => ({ ...s, automount: true }));
...
export function ServiceAccounts() {
  const [data, setData] = useState<SA[]>(fullData);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-[#1D2129]">服务账户</h1>
        <div className="flex items-center gap-2">
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild><Button size="sm" className="h-8 px-3 text-sm bg-[#165DFF] hover:bg-[#165DFF]/90 text-white"><Plus className="w-3.5 h-3.5 mr-1" />创建账户</Button></DialogTrigger>
            <DialogContent className="max-w-lg">
              <DialogHeader><DialogTitle className="text-base">创建服务账户</DialogTitle></DialogHeader>
              <div className="space-y-4 py-2">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5"><Label className="text-xs text-[#4E5969]">名称</Label><Input placeholder="如 my-sa" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="h-9 text-sm" /></div>
                ...
                <div className="flex items-center gap-2"><input type="checkbox" checked={form.automount} onChange={e => setForm({ ...form, automount: e.target.checked })} className="rounded" /><Label className="text-xs text-[#4E5969]">自动挂载 Token</Label></div>
              </div>
      ...
      <div className="bg-white rounded-lg border border-[#E5E6EB] overflow-hidden">
        <Table><TableHeader><TableRow className="bg-[#F7F8FA] hover:bg-[#F7F8FA]">
          ...
          <TableHead className="text-sm font-medium text-[#1D2129] h-10 px-4">密钥</TableHead>
        ...
        <TableBody>{paginated.length === 0 ? (<TableRow><TableCell colSpan={5} className="text-center py-16 text-[#86909C] text-sm">暂无服务账户数据</TableCell></TableRow>) : paginated.map(row => (
      ...
      <AlertDialog open={delOpen} onOpenChange={setDelOpen}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle className="text-base">确认删除？</AlertDialogTitle><AlertDialogDescription className="text-sm">即将删除服务账户 <span className="font-medium text-[#1D2129]">{delItem?.name}</span>，此操作不可恢复。</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="h-8 text-sm">取消</AlertDialogCancel><AlertDialogAction className="h-8 text-sm bg-[#F53F3F] text-white hover:bg-[#F53F3F]/90" onClick={confirmDel}>确认删除</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
    </div>
  );
}
