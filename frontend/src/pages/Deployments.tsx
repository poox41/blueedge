...
const parseKeyValueText = (value: string): Record<string, string> =>
  Object.fromEntries(
    value
      // Annotation values can contain JSON with commas (for example
      // kubectl.kubernetes.io/last-applied-configuration). Splitting on commas
      // turns fragments of that JSON into invalid Kubernetes annotation names.
      .split(/\n/)
      .map((item) => item.trim())
      .filter(Boolean)
      .map((item) => {
...
const friendlyWorkloadSaveError = (error: unknown): string => {
  const message = error instanceof Error ? error.message : "工作负载保存失败，请稍后重试";
  if (/metadata\.annotations|name part must consist of alphanumeric characters/i.test(message)) {
    return "保存失败：标签或注解名称格式不正确，请检查“标签与注解”后重试。";
  }
  if (/metadata\.labels/i.test(message)) {
    return "保存失败：标签名称格式不正确，请检查“标签与注解”后重试。";
  }
  if (/spec\.selector|field is immutable/i.test(message)) {
    return "保存失败：工作负载的选择器创建后不可修改。系统已保留原选择器，请刷新后重试。";
  }
...
export function Deployments() {
  ...
  const createFromForm = async (form: WorkloadForm) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "创建工作负载失败";
      setError(message);
  ...
  const updateFromWizard = async (item: Workload, form: WorkloadForm) => {
    ...
    try {
      ...
      const currentSelector = asRecord(currentSpec.selector);
      const currentMatchLabels = asStringRecord(currentSelector.matchLabels);
      const generatedTemplateLabels = asStringRecord(generatedTemplateMetadata.labels);
      const resource = normalizeDeploymentForUpdate(item, {
        ...
        spec: {
          ...currentSpec,
          ...generatedSpec,
          // A Deployment selector is immutable. Workloads created by an
          // EdgeApplication use controller-owned selector labels that differ
          // from the labels generated by this editor, so always retain it.
          selector: currentSpec.selector,
          template: {
            ...currentTemplate,
            ...generatedTemplate,
            metadata: {
...
function CreateWorkloadWizard({
  ...
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onCreate: (form: WorkloadForm) => void | Promise<void>;
  editItem?: Workload | null;
  initialLocation?: WorkloadWizardLocation;
  onUpdate?: (item: Workload, form: WorkloadForm) => void | Promise<void>;
}) {
  ...
  const create = async () => {
    ...
    setIsSubmitting(true);
    try {
      if (editItem && onUpdate) await onUpdate(editItem, nextForm);
      else await onCreate(nextForm);
    } catch (error) {
      setSubmitError(friendlyWorkloadSaveError(error));
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => nextOpen ? onOpenChange(true) : close()}>
      <DialogContent className="!flex max-h-[min(800px,calc(100vh-48px))] w-[min(600px,calc(100vw-48px))] max-w-none flex-col gap-0 overflow-hidden rounded-[24px] p-0 shadow-[0_24px_60px_rgba(16,24,40,0.18)] sm:max-w-[600px]" showCloseButton={false}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-6">
          ...
          {submitError && (
            <div className="mt-4 rounded-xl border border-[#fecaca] bg-[#fef2f2] px-4 py-3 text-sm leading-5 text-[#b91c1c]" role="alert">
              {submitError}
            </div>
          )}
        </div>
          ...
          <div className="flex w-full items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {step < 2 ? (
                ...
                <button type="button" onClick={() => void create()} disabled={isSubmitting} className="h-10 rounded-xl bg-[#0f172a] px-6 text-sm font-semibold text-white hover:bg-[#172033] disabled:cursor-not-allowed disabled:bg-[#9ca3af]">{isSubmitting ? "保存中..." : editItem ? "保存" : "创建"}</button>
              )}
            </div>
          </div>
        </DialogFooter>
